...
def create_masks(results_dir, metric, significance_cutoff, tfce_value_threshold, resolution, radius=None,
    ...
    masks_tfce = {hemi: copy.deepcopy(tfce_values[hemi]) for hemi in HEMIS}
    for hemi in HEMIS:
        print(
            f'{hemi} hemi mask size for threshold {threshold:.2f}: {np.mean(tfce_values[hemi] >= threshold):.2f}')
        masks_tfce[hemi][tfce_values[hemi] >= threshold] = 1
        masks_tfce[hemi][tfce_values[hemi] < threshold] = 0
        masks_tfce[hemi][np.isnan(tfce_values[hemi])] = 0
        masks_tfce[hemi] = masks_tfce[hemi].astype(np.uint8)
