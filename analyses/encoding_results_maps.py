...
def create_gifti_results_maps(args):
    ...
    for subj in args.subjects:
        ...
        for hemi in HEMIS:
            ...
            print(f"{os.path.basename(path_out)} max: {np.max(score_hemi_specific_lang):.2f}")
            ...
            print(f"{os.path.basename(path_out)} max: {np.max(score_hemi_specific_vision):.2f}")
            ...
            print(f"{os.path.basename(path_out)} max: {np.max(cross_lang_to_vision):.2f}")
