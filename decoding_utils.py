...
def split_vision_and_language_beta_files_2phase_glm(beta_dir, wbf=False):
    ...
    beta_file_addresses = list(
        sorted(glob(os.path.join(beta_dir, f'unstructured{mni_str}', '**', 'beta_*.nii'), recursive=True)))
    subdirs = {
        'train_images': os.path.join(beta_dir, f'betas_train_images{mni_str}'),
        'test_images': os.path.join(beta_dir, f'betas_test_images{mni_str}'),
        'train_captions': os.path.join(beta_dir, f'betas_train_captions{mni_str}'),
        'test_captions': os.path.join(beta_dir, f'betas_test_captions{mni_str}'),
        'imagery': os.path.join(beta_dir, f'betas_imagery{mni_str}'),
    }
    if wbf:
        subdirs['blank'] = os.path.join(beta_dir, f'betas_blank{mni_str}')
