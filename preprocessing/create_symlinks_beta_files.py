...
from utils import SUBJECTS, FMRI_BETAS_DIR, ADDITIONAL_TEST_UNSTRUCTURED_DIR_NAME
...
def create_symlinks_for_beta_files(betas_dir, unstructured_dir_name, subject, splits):
    ...
    for beta_path in tqdm(beta_file_addresses):
        ...
        if (split_name == IMAGERY) and (unstructured_dir_name == ADDITIONAL_TEST_UNSTRUCTURED_DIR_NAME):
        ...
        if split_name in splits:
            if split_name in ['blank', 'fixation', 'fixation_whitescreen']:
                ...
                stim_id = int(beta_name.split(split_name.replace('_weak', ''))[1].replace("_", ""))
