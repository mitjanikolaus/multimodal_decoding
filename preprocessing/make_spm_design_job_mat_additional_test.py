...
from data import IMAGERY, TEST_IMAGES_ATTENDED, TEST_CAPTIONS_ATTENDED, \
    TEST_IMAGES_UNATTENDED, TEST_CAPTIONS_UNATTENDED, SPLIT_IMAGERY
...
ID_TO_TRIAL_TYPE = {
    ...
    4: TEST_IMAGES_ATTENDED,
    5: TEST_CAPTIONS_ATTENDED,
    40: TEST_IMAGES_UNATTENDED,
    50: TEST_CAPTIONS_UNATTENDED
}
...
def get_attention_mod_condition_names(trial):
    ...
    if trial_type in [FIXATION, FIXATION_WHITESCREEN]:
        ...
        stim_id = trial['condition_name']
        assert trial_type in [TEST_IMAGES_ATTENDED, TEST_CAPTIONS_ATTENDED, TEST_IMAGES_UNATTENDED,
...
def preprocess_additional_test_event_files(event_files):
    ...
    for r_idx, event_file in enumerate(event_files):
        ...
        if (TRIAL_TYPE_TO_ID[TEST_IMAGES_ATTENDED] in trial_types) and (
                TRIAL_TYPE_TO_ID[TEST_CAPTIONS_ATTENDED] in trial_types):
...
def run(args):
    for subject in args.subjects:
        ...
        output_dir = str(os.path.join(args.output_dir, subject, ADDITIONAL_TEST_UNSTRUCTURED_DIR_NAME))
        ...
        for cond in [
            TEST_IMAGES_ATTENDED, TEST_CAPTIONS_ATTENDED, TEST_IMAGES_UNATTENDED,
            TEST_CAPTIONS_UNATTENDED, IMAGERY_INSTRUCTION
        ]:
        ...
        for cond in [TEST_IMAGES_ATTENDED, TEST_CAPTIONS_ATTENDED, TEST_IMAGES_UNATTENDED,
                     TEST_CAPTIONS_UNATTENDED]:
